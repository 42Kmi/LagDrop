...
SETTINGS=$(tail -n +1 "$DIR"/42Kmi/options_""${CONSOLENAME}"".txt|sed -E 's/^.*=//g') #Settings stored here, called from memory
...
##### Make Files #####
CONSOLE=$(echo "$SETTINGS"|sed -n 1p) ### Your Wii U's IP address. Change this in the "${CONSOLENAME}"ip.txt file
IPCONNECT=$(tail -n +1 /proc/net/ip_conntrack|grep "$CONSOLE"|sed -E "/(^#.*#$|^$|\;|#^[ \t]*$)|#/d") ### IP connections stored here, called from memory
...
WHITELIST=$(echo "$(tail -n +1 "${DIR}"/42Kmi/whitelist.txt|sed -E "/(^#.*#$|^$|\;|#^[ \t]*$)|#/d"|sed -E "s/^/\^/g"|sed -E "s/\^\^/^/g"|sed -E "s/$/|/g")"|sed -E "s/(\ *)//g"|sed -E 's/\b\.\b/\\./g'| sed -E 's/\n\r/\0/g'|tr -d '\r\n'|sed -E 's/\|$//g') ### Additional IPs to filter out. Make whitelist.txt in 42Kmi folder, add IPs there. Can now support extra lines and titles. See README
...
BLACKLIST=$(echo "$(tail -n +1 "${DIR}"/42Kmi/blacklist.txt|sed -E "/(^#.*#$|^$|\;|#^[ \t]*$)|#/d"|sed -E "s/^/\^/g"|sed -E "s/\^\^/^/g"|sed -E "s/$/|/g")"|sed -E "s/(\ *)//g"|sed -E 's/\b\.\b/\\./g'| sed -E 's/\n\r/\0/g'|tr -d '\r\n'|sed -E 's/\|$//g') ### Permananent ban. If encountered, automatically blocked.
